...
def _onnx_merge_levels(levels: Tensor, unmerged_results: List[Tensor]) -> Tensor:
    ...
    res = torch.zeros((int(levels.size(0)), int(first_result.size(1)),
                       int(first_result.size(2)), int(first_result.size(3))),
                      dtype=dtype, device=device)
    for level in range(len(unmerged_results)):
        index = torch.where(levels == level)[0].view(-1, 1, 1, 1)
        index = index.expand(int(index.size(0)),
                             int(unmerged_results[level].size(1)),
                             int(unmerged_results[level].size(2)),
                             int(unmerged_results[level].size(3)))
...
class MultiScaleRoIAlign(nn.Module):
    ...
    def infer_scale(self, feature: Tensor, original_size: List[int]) -> float:
        ...
        size = list(map(int,feature.shape[-2:]))
    ...
    def setup_scales(
        ...
        for shape in image_shapes:
            max_x = max(int(shape[0]), max_x)
            max_y = max(int(shape[1]), max_y)
    ...
    def forward(
        ...
        num_rois = len(rois)
        num_channels = int(x_filtered[0].shape[1])
