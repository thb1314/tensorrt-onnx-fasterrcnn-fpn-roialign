...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        boxes_per_image = [int(boxes_in_image.shape[0]) for boxes_in_image in proposals]
        if torchvision._is_tracing():
            boxes = proposals[0].detach()
            pred_bbox_deltas = box_regression.view(-1, int(box_regression.shape[-1] // 4), 4)

            # 1, anchor_num
            widths = boxes[:, 2] - boxes[:, 0]
            widths = widths.unsqueeze(dim=-1)
            heights = boxes[:, 3] - boxes[:, 1]
            heights = heights.unsqueeze(dim=-1)
            ctr_x = boxes[:, 0].unsqueeze(dim=-1) + 0.5 * widths
            # ctr_x = ctr_x.unsqueeze(dim=0)
            ctr_y = boxes[:, 1].unsqueeze(dim=-1) + 0.5 * heights
        ...
        if not torchvision._is_tracing():
            ...
            # 先仅支持 batch size = 1的
            assert pred_boxes.size(0) == boxes_per_image[0]
            pred_boxes_list = [pred_boxes] if int(pred_boxes.dim()) == 3 else pred_boxes
            pred_scores_list = [pred_scores] if int(pred_scores.dim()) == 2 else pred_scores

        all_boxes = []
        all_scores = []
        all_labels = []
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            boxes = box_ops.clip_boxes_to_image(boxes, tuple(map(int,image_shape)))

            # create labels for each prediction
            labels = torch.arange(num_classes, device=device)
            labels = labels.view(1, -1).expand(list(map(int, scores.shape)))
