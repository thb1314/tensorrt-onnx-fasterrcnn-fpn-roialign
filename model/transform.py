...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def forward(self,
        ...
        image_sizes = [list(map(int, img.shape[-2:])) for img in images]
    ...
    def normalize(self, image):
        ...
        mean = torch.as_tensor(self.image_mean, dtype=dtype, device=device).detach()
        std = torch.as_tensor(self.image_std, dtype=dtype, device=device).detach()
    ...
    def resize(self, image, target):
        ...
        h, w = list(map(int,image.shape[-2:]))
        if self.training:
        ...
        bbox = target["boxes"]
        bbox = resize_boxes(bbox, (h, w), tuple(map(int, image.shape[-2:])))
        target["boxes"] = bbox

        if "keypoints" in target:
            keypoints = target["keypoints"]
            keypoints = resize_keypoints(keypoints, (h, w), tuple(map(int, image.shape[-2:])))
    ...
    def _onnx_batch_images(self, images, size_divisible=32):
        ...
        max_size = tuple(map(int, max_size))
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        max_size = self.max_by_axis([list(map(int, img.shape)) for img in images])
...
def resize_boxes(boxes, original_size, new_size):
    ...
    ratios = [
        ...
        torch.as_tensor(s_orig, dtype=torch.float32, device=boxes.device)
        for s, s_orig in zip(new_size, original_size)
    ]
